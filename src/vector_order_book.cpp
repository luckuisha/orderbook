...
OrderResult VectorOrderBook::process_order(const Order& order) {
...
OrderResult VectorOrderBook::cancel_order(uint64_t id) {
...
OrderResult VectorOrderBook::modify_order(uint64_t id, double new_price, uint32_t new_quantity) {
    auto it = id_lookup_side_.find(id);
    if (it == id_lookup_side_.end()) return OrderResult(OrderResult::OrderStatus::Rejected);
...
uint32_t VectorOrderBook::quanitity_at_price(double price, Order::Side side) {
...
void VectorOrderBook::display_order() const {
